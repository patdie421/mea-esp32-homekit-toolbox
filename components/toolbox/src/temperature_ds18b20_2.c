...
static int _add_new(uint32_t addr0, uint32_t addr1)
{
   for(int i=0;i<MAX_SENSORS;i++) {
      if(_temperature_ds18b20_data_2[i].state == TEMP_NOT_SET) {
         _temperature_ds18b20_data_2[i].cb=NULL;
         _temperature_ds18b20_data_2[i].userdata=NULL;
         _temperature_ds18b20_data_2[i].userid=-1;
         _temperature_ds18b20_data_2[i].addr0=addr0;
         _temperature_ds18b20_data_2[i].addr1=addr1;
         _temperature_ds18b20_data_2[i].current=-999.0;
         _temperature_ds18b20_data_2[i].last=-999.0;
         _temperature_ds18b20_data_2[i].state=TEMP_NOT_READ;
         return i;
      }
   }
   return -1;
...
static void _force_status_not_found()
{
   for(int i=0;i<MAX_SENSORS;i++) {
      if(_temperature_ds18b20_data_2[i].state!=TEMP_NOT_SET) {
         _temperature_ds18b20_data_2[i].state=TEMP_NOT_FOUND;
      }
   }
}
...
int temperature_ds18b20_init_2()
{
   ...
   for(int i=0;i<MAX_SENSORS;i++) {
      ...
      _temperature_ds18b20_data_2[i].userdata=NULL;
      _temperature_ds18b20_data_2[i].userid=-1;
      _temperature_ds18b20_data_2[i].addr0=addrs[i]>>32;
      _temperature_ds18b20_data_2[i].addr1=addrs[i];
      _temperature_ds18b20_data_2[i].current=0.0;
      _temperature_ds18b20_data_2[i].last=0.0;
      if(i<sensor_count) {
         _temperature_ds18b20_data_2[i].state=TEMP_NOT_READ;
      }
      else {
         _temperature_ds18b20_data_2[i].state=TEMP_NOT_SET;
      }
   }
   return sensor_count;
}
...
int temperature_ds18b20_remove_by_addr_2(uint32_t addr0, uint32_t addr1)
{
   ...
   if(id!=-1) {
      ...
      _temperature_ds18b20_data_2[id].userdata=NULL;
      _temperature_ds18b20_data_2[id].userid=-1;
      _temperature_ds18b20_data_2[id].addr0=0;
      _temperature_ds18b20_data_2[id].addr1=0;
      _temperature_ds18b20_data_2[id].current=0.0;
      _temperature_ds18b20_data_2[id].last=0.0;
      _temperature_ds18b20_data_2[id].state=TEMP_NOT_SET;
      return 1;
   }
   return 0;
}
...
int temperature_ds18b20_set_cb_by_addr_2(uint32_t addr0, uint32_t addr1, temperature_ds18b20_callback_2_t _cb, void *_userdata, int8_t _userid)
{
   int id=_find_addr(addr0, addr1);
   if(id!=-1) {
...
{
  if(id<MAX_SENSORS && _temperature_ds18b20_data_2[id].state!=TEMP_NOT_SET) {
            ...
            for(int j = 0; j<sensor_count; i++) {
               ...
               if(id!=-1) {
                  ...
                  if(_temperature_ds18b20_data_2[id].cb) {
                     _temperature_ds18b20_data_2[id].cb(_temperature_ds18b20_data_2[id].current, _temperature_ds18b20_data_2[id].last, _temperature_ds18b20_data_2[id].userdata);
                  }
               }
               else {
